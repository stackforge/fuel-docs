...
.. code-block:: ini
   ...
   - name: additional_service:fuel-plugin-example
     ...
     requires:
       - name: storage:block:ceph
         message: "Example component requires Ceph as a storage backend"
       - name: storage:ephemeral:ceph
         message: "Example component requires Ceph as an ephemeral storage backend"
     incompatible:
...
defines the components that cannot be be enabled with this component.
...
.. note:: Currently, there is no association between the settings bound
          ...
          restrict each other via the mechanism described in

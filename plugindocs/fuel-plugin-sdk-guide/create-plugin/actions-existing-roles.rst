...
.. tip:: Here is how plugin live update works. When a plugin is installed,
         ...
         core tasks, not related to your plugin, but this is outside of the
         scope of this guide.
...
In our example, we used the asterisk '*' as the value for the keyword role.
To run the tasks only on some roles, but not the others, replace the value
with the list of roles on which to run; for example:
...
from shell, but Puppet is most widely used to write Fuel plugins and there is
support for Puppet tasks.
...
.. toctree::
   ...
   create-plugin/actions-existing-roles/
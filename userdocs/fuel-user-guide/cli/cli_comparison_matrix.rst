...
The Fuel command-line interface ``fuel2`` has been changed to simplify usage
...
.. csv-table:: **Fuel CLI commands comparison matrix**
   ...
   ``fuel2 network-group show``, Show a network group, None
   ...
   ``fuel2 node attributes-download``, Download a node attributes, None
   ``fuel2 node attributes-upload``, Upload a node attributes, None
   ...
   ``fuel2 node list``, Show list of all available nodes, ``fuel node``
   ``fuel2 node list-vms-conf``, Show a list VMs for a node, None

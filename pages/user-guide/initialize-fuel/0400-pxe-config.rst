...
the interface where Fuei web UI will be accessible. eth2 should use DHCP.
...
* Save and Continue - performs built-in tests and , if test passed successfully,
...
* Save and Quit - performs built-in tests and , if test passed successfully,
  saves all the current settings from all sections, except the first one,
  Network Setup, which has its own Apply button.

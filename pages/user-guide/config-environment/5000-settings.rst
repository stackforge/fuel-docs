...
* Security:  reset login credentials for Horizon,
  set/reset login credential for vCenter,
  set the Public Key to use on deployed nodes
...
* Configure VLAN Splinters for environments
  running CentOS on the slave nodes
  with a network topology that uses VLANs.

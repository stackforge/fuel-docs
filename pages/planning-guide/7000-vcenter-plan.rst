...
- Volumes that are created by Cinder appear as SCSI disks. To be able
  read/write that disk user need to be sure that operating system inside
  instance supports SCSI disks. CirrOS image that is shipped with Fuel
  supports only IDE disks, so even if volume would be attached to it,
  CirrOS would not be able to use it.

...
- Select peripheral hardware that is supported
  ...
  that you are using for the environment nodes
  (see :ref:`linux-distro-plan`)
  and for the VM instances that will be deployed.

  Integrating new drivers into the Mirantis OpenStack distribution
  is difficult at this time.
  Note also that proprietary drivers
  that may be included in the standard Linux distributions
  may not be included in the Mirantis OpenStack distribution

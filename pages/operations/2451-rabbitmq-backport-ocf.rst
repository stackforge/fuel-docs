...
#. Update the configuration of the p_rabbitmq-server resource for
   ...
   - To the `params` stanza:
     ...
     .. note:: The ``command_timeout`` parameter value is given for Ubuntu OS.
        For Centos, this parameter should be set to ``-s KILL``

     .. note:: Use ``some_param="some_value"`` notation or for the XML case:
        ::

            <nvpair id="p_rabbitmq-server-instance_attributes-some_value" name="some_param" value="some_value"/>
  ...
  - To the `meta` stanza:
    ...
    - Set the ``failure-timeout`` to ``"360s"``

  - To the `op` stanzas:

    - Set the ``notify interval`` to ``"0"`` and the ``timeout`` to ``"180"``
    - Set the ``start interval`` to ``"0"`` and the ``timeout`` to ``"360"``

...
.. code-block:: bash
   ...
   container=postgres
   container_id=$(sqlite3 /var/lib/docker/linkgraph.db "select entity_id from edge where name like '%$container%'")
   echo $container_id
   #should look like:
...
Verify that data is present in mounted directory:

.. code-block:: bash

   ls -la /mnt/postgres_recovery/rootfs/var/lib/pgsql/9.3/data/

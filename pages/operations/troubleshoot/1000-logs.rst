...
The /etc/logrotate.d/10-fuel-docker.conf file is called by /etc/cron.daily/logrotate
and provides daily rotation of logs. The /etc/logrotate.d/20-fuel-docker.conf
file is called by /etc/cron.hourly/logrotate and provides an additional layer
of rotation based on maximum log file size criteria. This ensures that the free space
of /var/log is never exhausted with an apropriate consideration for free space planning
matching the given log rotation settings.

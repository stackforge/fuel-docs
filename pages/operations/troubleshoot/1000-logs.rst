...
* The `/etc/logrotate.d/10-fuel-docker.conf` file is
  called by the logrotate script every 15 minutes.
...
* To purge old templates when doing backporting of the reworked logrotate configuration
  for older Fuel releases you need to run:

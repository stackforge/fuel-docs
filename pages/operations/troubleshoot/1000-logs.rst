...
* The `/etc/logrotate.d/fuel.nodaily` file is
  called by the logrotate script every 30 minutes.

* The rotation of logs is performed when meeting one of the criteria: age - if
  it is more than a week and a file is bigger than 10M on master or 5MB on
  remote nodes; size - if it exceedes 100M on master or 20M on remote nodes,
  regardless its age

* You can run a quick test to check if the logrotate script works.

  Find the biggest file, and check the first and the last line:
  ...
  If it is older than your rotation schedule and bigger then maxsize,
  than logrotate is not working correctly.

  To debug that, an operator can run:
...
* You can find an example of a writing rate evaluation for the neutron-server log file:
  https://bugs.launchpad.net/fuel/+bug/1382515/comments/35

* When backporting of the reworked logrotate configuration
  for older Fuel releases you need to purge old template files:

  .. code-block :: sh

     rm /etc/logrotate.d/{1,2}0-fuel*

  The script `/usr/bin/fuel-logrotate` is needed as well as a new cron job to perform the rotation with it.

...
* The logs are rotated when one of the following criteria is met:

  * age - if the log has not been rotated in more than the specified period of
    time (weekly by default), and the file is larger than 10MB on master or
    5MB on remote nodes;

  * size - if a log file exceeds 100MB on master or 20MB on remote nodes.

.. warning::
  Fuel enforces the following global changes to logrotate configuration:

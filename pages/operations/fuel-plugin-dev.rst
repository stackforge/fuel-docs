...
* *post_deployment* - this section has only one task which is taken from
  *tasks.yaml* file; *uids* field has a single node UID to be executed
  (irina: to be used/involved into this task?)
  (eli: I'm not sure here, the point is *uids* field has a list of nodes where we should run/execute
  a particular task)

  on. In this example, *tasks.yaml* file has "role: ['controller']" and
  this role is assigned to controller with UID 7.

* *deployment_info* - the section has regular
  (irina: what do you mean under regular? default?)
  (eli: it's not a default, I'll try to explain what I meant, by regular I meant usual data
  which we pass even if there is no plugins at all, this section contains all configuration
  information which is required for deployment and it's not related to plugins)
  deployment data for each node.
...
UI elements are described in *environment_config.yaml* file, to check
how your plugin looks on UI build plugin, install and create environment.
...
To show how it works, we create a simple plugin with error in
deployment script.
...
Add error in the default deployment script
...
Use UI or CLI to create environment, enable plugin on UI Settings tab,
add several nodes, in my example I have one node with Controller role
and another with Compute and Cinder roles.

Check that Nailgun generates right data which user can configure on UI.
...
.. note::
    ...
    Also the command above is useful when you don't know how your data
    from settings tab will look like in */etc/astute.yaml* file on
    target nodes.

To reduce a time of debugging, lets provision two nodes without deployment
...
If you use virtual machines make snapshots of you target nodes.
...
The deployment fails because of our changes in deploy.sh script.

To fix the problem and check that our fix works we should not
rebuild and reinstall a plugin, we can fix the script on the master node
*/var/www/nailgun/plugins/fuel_plugin_name-1.0.0/deployment_scripts/deploy.sh*
...
If you run it again deployment passes
...
In the same way you can (without plugin reinstallation) you can change
*/var/www/nailgun/plugins/fuel_plugin_name-1.0.0/tasks.yaml* file,
but in this case at least run
*fpb --check /var/www/nailgun/plugins/fuel_plugin_name-1.0.0/*
to perform static checks that your tasks have valid format

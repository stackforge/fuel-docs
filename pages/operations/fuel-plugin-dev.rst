...
4. Use UI or CLI to create environment:

.. code-block:: bash

   fuel env create --name test --rel 1 --mode multinode --network-mode nova

5. Enable the plugin on Fuel UI Settings tab and then add several nodes.
...
6. Check that Nailgun generates right data that a user can configure on Fuel UI:
...
we can see that the file for target node contains plugin's data
...
and if you use virtual machines, make snapshots of your target nodes.
...
The deployment fails with the next message:

::

    Deployment has failed. Method deploy. Failed to deploy plugin fuel_plugin_name-1.0.0.

8. It fails due to the changes in *deploy.sh* script that you made in
   step 2. Lets assume that we don't know what happened and debugg the problem
...
All deployment scripts of the plugin are copied in separate directory on the
target node, for our plugin it's */etc/fuel/plugins/fuel_plugin_name-1.0.0/*.
...
Now we can see that deployment failed because of error (= non-zero) exit code.

Also in the orchestrator logs */var/log/docker-logs/astute/astute.log*
...
9. To fix the problem and check that the proposed solution works, edit the
...
10. If you run the deployment again, it goes successfully:
...
.. warning::
    ...
    your plugin works correctly. The reason for this is Fuel can
    run deployment of your plugin several times, for example if
    first deployment failed, also your deployment scripts can be run during
    OpenStack patching.

11. To make sure that plugin works without errors, revert snapshots
...
12. In the same way with no plugin reinstallation, you can edit

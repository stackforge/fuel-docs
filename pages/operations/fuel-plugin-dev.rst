...
This new feature allows extending Fuel in a more flexible manner:
after Fuel upgrade there is no need to apply patches manually and
support them.

Core and Experimental plug-ins are enabled.

Core plug-ins, `GlusterFS <http://www.gluster.org/
documentation/About_Gluster>`_ and `LBaaS Neutron <https://wiki.openstack.org/wiki/Neutron/LBaaS/
PluginDrivers>`_, are kept in `fuel-plugins <https://github.com/stackforge/fuel-plugins>`_ repo. 

Experimental plug-ins are kept in `Mirantis fuel-plugins <https://github.com/Mirantis/fuel-plugins>`_ repo.

Certificatin
Once plug-in is ready,
you should upload it there.
It will be slightly tested, built and published to Mirantis web page, so that any
user would be able to download and install it.
...
#. Build your plug-in:
  ...
  Now you can install your plug-in :ref:`install-plugin`.


Plug-in structure in details
----------------------------
...
* *title* - human-readable name for the plug-in that will appera
  on the Fuel web UI.
...
command returned zero/non-zero exit code.
...
Where is Fuel Plug-in Builder source code?
++++++++++++++++++++++++++++++++++++++++++

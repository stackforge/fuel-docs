...
* *fuel_version* - a list of plugin-compatible (irina: did I understand the
  idea correct here? we should know if plugin is compatible?)versions of Fuel.
...
To check how your built plugin looks on Fuel web, install and create environment:
...
9. It fails due to the changes in **deploy.sh** script that you made in
...
10. All plugin deployment scripts are copied to the separate directory on the
...
11. Now we can see that deployment failed because of (= non-zero) exit code error.

12. You can see an error in **/var/log/docker-logs/astute/astute.log** Orchestrator logs:
...
.. warning::
    ...
    your plugin works correctly. The reason for this step is the following:

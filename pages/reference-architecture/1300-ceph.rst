...
Settings for Ceph configuration are located
under the */etc/puppet/modules/ceph* directory.
...
- Initialize a Ceph monitor (MON) service
  on each configured Controller node.
  At this time, Fuel does not have a separate
  Ceph-MON role that can be configured.

Mirantis OpenStack 4.0 and later
includes enhancements to the Ceph copy-on-write capability
that are not included in the OpenStack releases:

* Copy-on-write and live migrations just work on Mirantis OpenStack;
  it is not necessary to first create an RBD backed bootable volume
  from a raw image.
...
* Launching an image on Mirantis OpenStack
  ...
  for standard OpenStack deployments.
...
* Ceph on Mirantis OpenStack stores ephemeral drives
  ...
  Standard OpenStack deployments can only store ephemeral drives
...
.. include:: /pages/reference-architecture/ceph/5000-cephmon.rst
.. include:: /pages/reference-architecture/ceph/6000-live-migration.rst

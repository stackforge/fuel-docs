...
* **interfaces**ÑA hash of NICs and their low-level/physical parameters.
  You can set an MTU and the 'VLAN splinters' feature here.
* **provider**ÑSet to 'ovs' for Neutron.
* **endpoints**ÑA hash of network ports (OVS ports or NICs) and their IP
  settings.
* **roles**ÑA hash that specifies the mappings between the endpoints and
  internally-used roles in Puppet manifests ('management', 'storage', and so on).
* **transformations**ÑAn ordered list of OVS network primitives.
...
* **add-br**ÑTo add an OVS bridge to the system
* **add-port**ÑTo add a port to an existent OVS bridge
* **add-bond**ÑTo create a port in OVS bridge and add aggregated NICs to it
* **add-patch**ÑTo create an OVS patch between two existing OVS bridges

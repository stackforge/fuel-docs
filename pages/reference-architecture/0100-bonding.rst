...
* One side bonding does not require any special features support from the switch side. Linux handles it using a set of traffic balancing algorithms.
...
3. Click "Bond interfaces". Now you can select appropriate bonding mode from drop-down list "Mode".
...
You can add an interface(s) to bond. Select one bond and interface(s) to add
and click "Bond Interfaces". Interface(s) can be removed from bond when bond has

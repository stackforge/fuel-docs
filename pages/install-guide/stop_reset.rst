...
**IMPORTANT NOTE:** At the current moment, deployment cannot be interrupted
during the provisioning stage. This means that a user can click on "Stop
deployment" while nodes are provisioning, but they will be rebooted back to
bootstrap only when OS installation is complete.

...
2. **[Q]** Is MySQL with Galera an active/active HA? Does it support
   ...
   **[A]** Yes, MySQL+Galera is a true multi-master solution. Although MySQL+Galera
   ...
   to manage MySQL+Galera. Pacemaker provides a Virtual IP address (VIP) for the    MySQL+Galera cluster and multi-master writes are not used because Openstack
   configuration does not support it and can only work with a single connection.

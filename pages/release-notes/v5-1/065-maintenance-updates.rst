...
* A storage quota bypass flaw was found in OpenStack Image (glance).
  ...
  deliberately fill the backing store and cause a denial of service. The fix
  removes the chunks left from an image that was deleted in the ``saving``
  status. See `LP1414685`_.

* If an image is deleted during the file upload when you want to update the
  image status from ``saving`` to ``active``, it is expected to get a
  `Duplicate` error and delete stale chunks after that. But if a user's token
  is expired, there will be an `Unauthorized` exception and chunks will stay
  in the store and clog it. Only Glance setups configured with
  ``user_storage_quota`` are affected. The fix removes chunks left from an
  image whose uploading was canceled due to the `Unauthorized` exception.
  See `LP1497984`_.
...
* By setting an image location, an authenticated user can
  ...
  deleting any file via the glance-api server. See `LP1403102`_.

* The path traversal vulnerabilities in Glance were not fully patched
  in OSSA 2014-041. By setting an image location to a `filesystem://`
  scheme, an authenticated user can still download or delete any file
  from the Glance server to which the user running the Glance process has
  access to. Only setups that use Glance V2 API are affected by this flaw.
  The fix ensures that `file`, `filesystem`, and `swift+config` URI schemes
  are not allowed when setting the ``location`` field. See `LP1514467`_.
...
* By overwriting an image with a malicious ``qcow2`` header, an authenticated
  user may mislead the ``cinder upload-to-image`` action resulting in a
  disclosure of any file from the Cinder server. All Cinder setups are
  affected. The fix restricts referencing backing files as a security measure

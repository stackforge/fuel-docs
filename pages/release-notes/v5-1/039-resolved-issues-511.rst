...
* Creating volume from image no longer performs full data copy
  when Ceph-OSD is used as the storage backend
...
* To upload CirrOS imaged on Ubuntu,
  the "**.**" command is now used instead of the **source** command.
...
* When running Puppet noop tests on fuel-library
  the "could not find resource" error no longer occurs.
...
* If default **admin** name is changed to a custom name,
  the tenant now reports the correct custom name.

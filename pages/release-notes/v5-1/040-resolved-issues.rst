...
----------------------------------------------
RabbitMQ RabbitMQ now uses the built-in autoheal facility to correctly manage cluster partitions.
...
* Ceph nodes can be added into environment for vCenter.
  See `LP1351288 <https://bugs.launchpad.net/fuel/+bug/1351288>`_.

* When controller and Cinder are located on the same node
  and the deployment type is 'vCenter with VMDK', deployment now completes successfully.
...
* When Cirros image is registered in Glance during vCenter deployment, *incorrect
  VMware adapter type* error no longer appears. See `LP1352898 <https://bugs.launchpad.net/fuel/+bug/1352898>`_.

* Now it is possible to deploy OpenStack with vCenter without specifying vCenter credentials.
...
* Connection is no longer closed by remote host after stopping
  deployment at the end of provisioning.
...
* As Ceph nodes can not be safely updated in 5.1 release,
  mind the following:

1. If you want to update Ceph manually, then new packages must be installed and
   monitors must be restarted.

2. Once all monitors are restarted, update the code on the OSD nodes one by one.
   Wait until OSD nodes have rebuilt cleanly after OSD restart.

3. To check if any non-active-clean states are present, see `ceph pg dump` command
   output. If unclean pgs are in Ceph dump output,
   you should resolve this before considering a Ceph update..
...
* During patching, failed resources are now migrated from the node.
  See `LP1365635 <https://bugs.launchpad.net/fuel/+bug/1365635>`_.
...
* OSFT of Cinder nodes now work correctly after failover.
...
* Upgrade now fails if there is no connectivity to Keystone
...
* Dnsmasq logs now appear in Cobbler logs directory. See `LP1357408 <https://bugs.launchpad.net/fuel/+bug/1357408>`_.
...
* The :ref:`VMDK driver<vmdk-term>` option can now be enabled in
  Fuel web UI.
...
* The wizard configuration now correctly uses the `Weight` parameter.
...
* MySQL log settings correctly send logs to Fuel Master now on Ubuntu.
...
* OSFT now display correct check status.
...
* Murano, Sahara and Heat are now deployed with usernames,
  inluding @example.com email address.
...
* Separate *openstack-patching.yaml* file is used for upgrading tarball.
  See `LP1353369 <https://bugs.launchpad.net/fuel/+bug/1353369>`_.

* In a specific version requirement is stated for the package, that version will be installed instead of
  stated for a given package.
...
* OSTF test no longer fail during upgrade.
  See `LP1342045 <https://bugs.launchpad.net/fuel/+bug/1342045>`_.

* CentOS and Ubuntu packages were provide for Ceph v0.80.1;ceph-deploy packages were updated to 1.5.2.
  See `LP1324235 <https://bugs.launchpad.net/fuel/+bug/1324235>`_.

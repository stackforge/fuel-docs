...
* As Ceph nodes can not be safely updated in 5.1 release,
  ...
  - If you want to update Ceph manually, then new packages must be installed and
    monitors must be restarted.

  - Once all monitors are restarted, update the code on the OSD nodes one by one.
    Wait until OSD nodes have rebuilt cleanly after OSD restart.

  - To check if any non-active-clean states are present, see `ceph pg dump` command

...
----------------------------------------------
RabbitMQ RabbitMQ now uses the built-in autoheal facility to correctly manage disk partitions.
...
* Ceph node can be added into environment for vCenter.
...
* Rollback works on MongoDB nodes.
...
* Radosgw successfully starts on CentOS.
...
1. If you wand to update Ceph manually, then new packages must be installed and
   monitors must be restarted.

2. Once all monitors are restarted, update the code on the OSD nodes one by one.
   Wait until OSD nodes get rebuilt cleanly after OSD restart.

3. To check if any non-active-clean states are present, see `ceph pg dump` command
   output. If unclean pgs are in Ceph dump output, reconsider Ceph update.
...
* Galera always synchronizes on the slave nodes.
...
* RFC syslog option is now included in compute node manifest.
...
* The :ref:`VMDK driver<vmdk-term>` option is enabled in Fuel web UI.
...
* Refresh for Galera mySQL service was enabled in manifest.
...
* In a specific version requirement is stated for the package, that version will be installed instead of "latest" one.

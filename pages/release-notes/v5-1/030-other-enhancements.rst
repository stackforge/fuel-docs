...
- The vCenter Virtual Machine Disk (VMDK) can be utilized
  as a back-end for Cinder utilizing the standard upstream VMWare VMDK Cinder driver.
...
The Zabbix server can be selected as a role to be deployed onto a discovered node;
Zabbix agents are automatically deployed
onto all other OpenStack nodes in the environment.  

Support for ISER and SR-IOV based networking over Mellanox hardware (experimental)
----------------------------------------------------------------------------------
...
- Install and configure Mellanox configurations for OpenStack
  (included in ML2 plugin) for high performance VM links
- Test the environment utilizing Mirantis OpenStack Health Checks

The included Ceph distribution has been updated to Firefly
----------------------------------------------------------
...
Mirantis does not necessarily support these packages or guarantee that they work.

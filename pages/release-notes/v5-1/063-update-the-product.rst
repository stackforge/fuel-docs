...
A special script can be used for the automated update of the nodes
in all or particular environments:

       `mos_apply_mu.py <https://github.com/Mirantis/tools-sustaining/raw/master/scripts/mos_apply_mu.py>`_

This script downloads updates repository and then updates all nodes
simultaneously. It should be run on the Fuel Master node in the following
manner in order to update nodes in the environment `X` given that Fuel
user name is `user_name`, password is `user_password` and tenant_name is `tenant_name`::

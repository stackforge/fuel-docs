...
* Copying files, necessary for correct work environment are not copied;
  that leads to failed tempest tests.
  See `LP1387802 <https://bugs.launchpad.net/bugs/1387802>`_.
...
* After Fuel upgrade, Rsync Docker container uses old puppet manifests while generating xinetd configuration.
...
* Logs of agent rescheduling script do not contain enough information on
  found dead agents and debugging issues.
...
* When using 10gig interfaces, the kernel can not turn interfaces in promisc mode due to generic segmentation offload.
  Every time this error leads to agent migration to another host;
  instances leave their IP addresses because they have no more access to DHCP server.

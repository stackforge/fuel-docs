...
* If you add a new application to an environment with a *Deploy FAILURE*
  ...
  does (*Deploy FAILURE* instead of *Ready*, although you have not
  started to deploy this application yet). Once the deployment of
  environment is successful, the application changes its status to the
  correct one (*Ready*). See `LP1441246`_.

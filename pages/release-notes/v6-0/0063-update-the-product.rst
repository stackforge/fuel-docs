...
This script updates all nodes one by one and should be run on the Fuel Master node in the following manner in order to update nodes in the environment `X` given that IP address of the Fuel Master node is 10.20.0.2::
       ...
       python mos_apply_mu.py --env-id=X --update --master-ip=10.20.0.2 --user=admin --pass=admin --tenant=admin

To update all environments, use `--all-envs` option::

       python mos_apply_mu.py --update --all-envs --master-ip=10.20.0.2 --user=admin --pass=admin --tenant=admin

The status of the update process can be checked by using `--check` option::

       python mos_apply_mu.py --check --all-envs --master-ip=10.20.0.2 --user=admin --pass=admin --tenant=admin
...
Manual updating procedure for Linux CentOS- and Ubuntu-based nodes in a closed environment
--------------------------------------------------------------------------------------------
...
.. note::
      After updating Mirantis OpenStack services need to be restarted manually, ``p_neutron-l3-agent``, ``p_neutron-metadata-agent``,
      ``p_neutron-dhcp-agent``, ``p_neutron-plugin-openvswitch-agent``, ``p_heat-engine`` services on controller node should be
      restarted using ``crm resource restart``, other - using native operating system's service manager.
      On compute node all OpenStack services should be restarted with operating system's service manager.

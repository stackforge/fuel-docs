...
.. note::
      After updating on all nodes OpenStack services need to be restarted manually in order for updates to have effect.
      HA-enabled services on controller nodes should be restarted using ``crm resource restart`` while non-HA services should be restarted using native OS service manager.
      ...
      On compute nodes all OpenStack services should be restarted using native OS service manager.

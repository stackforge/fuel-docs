...
.. note::
      After updating all Mirantis OpenStack services need to be restarted manually. The services managed with ``pacemaker`` should be restarted using ``crm resource restart``. There is no need in restarting of non-OpenStack services.

...
Run the following command on the Fuel Master node to obtain a local mirror of updates repository::
...
You can find your environment id in the list of available environments by running the following command on the Fuel Master node::
...
.. note::
      The script uses keystone’s authentication in order to obtain nodes list from Fuel API, thus access credentials must be provided.

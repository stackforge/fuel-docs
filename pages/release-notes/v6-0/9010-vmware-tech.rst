...
* When using vCenter as a hypervisor, vCenter can be used as a storage backend for Cinder.
  Fuel now lists **VMWare VCenter** as a role when available.
...
* In earlier Fuel releases, 1-N mapping between nova-compute service
  ...
  In most cases, single nova-compute service instance uses many vSphere clusters, managed by a single vCenter. It is planned to introduce this feature in 6.1 to allow
...
* When vCenter is selected as the hypervisor,
  ...
  See `1-1  <https://blueprints.launchpad.net/fuel/+spec/1-1-nova-compute-vsphere-cluster-mapping>`_ blueprint.
...
* When using vCenter as a hypervisor, vCenter can be used as a storage backend for Cinder.
  ...
  It is just a naming problem, so installing this role will enable Cinder VMDK feature.
  See `LP1383224 <https://bugs.launchpad.net/fuel/+bug/1383224>`_.

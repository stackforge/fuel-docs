...
* VMware DataStore option was added to the **Storage Backends**
...
* vCenter does not support Ceilometer.
  See
  `Enable Ceilometer for vCenter <https://blueprints.launchpad.net/fuel/+spec/ceilometer-support-for-vcenter>`_ blueprint.
...
* When we create a new OpenStack environment, the Fuel web UI does not prompt user to configure
...
* Speed of copying images between vCenter and non-VMDK Glance backends can be dramatically low.
  VMDK backend for Glance would fix this problem, but
  this approach is useless if multihypervisor environment is enabled.
  See
  `VMware vCenter templates available as Glance images blueprint <https://blueprints.launchpad.net/glance/+spec/hypervisor-templates-as-glance-images>`_.
...
* On CentOS in HA mode on vCenter machine, cluster deployment finishes
  ...
  To work around this problem, do not delete controller; otherwise, your environment will crash.
  See `LP1371638 <https://bugs.launchpad.net/fuel/+bug/1371638>`_.

* When a node with Controller role is added to the environment,
  Cinder LVM stays unavailable. To work this problem around
  and unblock UI element, follow instructions described
  in :ref:`assign-roles-vcenter-ug`.
...
* Fuel 6.0 supports VLAN manager, but the following problem occurs:
  when vCenter is deployed with VLAN manager in HA mode,
  no clean-up is provided.
  See `LP1392719 <https://bugs.launchpad.net/fuel/+bug/1392719>`_.

* Instance snapshot fails vCenter is used as the backend for Glance.

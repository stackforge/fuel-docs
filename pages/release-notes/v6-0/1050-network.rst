...
* When multiple node groups are used, IP addresses
  are allocated properly.
  See `LP1389160 <https://bugs.launchpad.net/bugs/1389160>`_.
...
* After environment is shut down, Neutron server no longer
  sends error messages in the logs.
...
* Multiple floating ranges not working when using Neutron.
  Deployment succeeds but only a single allocation_pool gets allocated to the floating subnet.
  Currently, this problem has several workarounds:
  divide floating IP addresses into different subnets, where every subnet has 1 allocation_pool;
  bypass API and directly manipulate mysql db, but it may be dangerous for the 
  environment (for more information,
  read `Solve Fragmented floating ips problem <http://samos-it.com/posts/neutron-multiple-allocation-pools-single-subnet.html>_`).
  See `LP1371363 <https://bugs.launchpad.net/bugs/1371363>`_.
...
* After HAProxy failover, it is impossible to ping an instance
  with the assigned floating IP.
  See `LP1391152 <https://bugs.launchpad.net/bugs/1391152>`_.
...
* When dnsmasq runs out of IP addresses, no feedback is provided.
  To work around this problem, you should login to Cobbler

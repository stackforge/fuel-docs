...
which may cause some Fuel services to fail
when the Fuel Master node redirects some requests to dead containers.
To fix this,
you can manually remove old iprules from the Fuel Master node
...
Cobbler may not get provisioning information for all Ubuntu nodes
-----------------------------------------------------------------

Cobbler may not get provisioning information for all Ubuntu nodes
when deploying larger environments
because of issues retrieving this information.
from the Ubuntu preseed facility.
Problems were observed when deploying an environment with 20 nodes.

...
As a general rule, the environment should contain
at least one MongoDB role for each Controller node,
so an HA environment should contain at least three MongoDB roles.
...
When installing Ubuntu on more than 10 nodes in parallel (problem observed at
20 nodes), Cobbler can take too long to generate preseed data for all nodes,
and some of the nodes will fail to provision. Adding 10 nodes at a time to the
environment mitigates this problem.
...
Adding Compute node after upgrading Fuel may fail
-------------------------------------------------

If one deploys an OpenStack environment using Fuel 5.0
and using Ceph for image storage,
then upgrades to Fuel 5.0.1,
an attempt to add additional Compute nodes to the environment may fail
with a ceph-deploy error.
See `LP1353497 <https://bugs.launchpad.net/fuel/+bug/1353497>`_.
...
See `LP1318936 <https://bugs.launchpad.net/fuel/+bug/1318936>`.

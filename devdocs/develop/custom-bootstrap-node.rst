...
.. warning:: Operating on active bootstrap may lead to the case when
 a suddenly rebooted node fails to download bootstrap images ``initrd``
 and ``root.squashfs``. The rebooted node will repeat the attempts until
 ...
 :command:`fuel-bootstrap build` and :command:`fuel-bootstrap import`.
 Modify the ``initrd`` and ``root.squashfs`` files as described
 below and activate this new image using the command
 :command:`fuel-bootstrap active <your-bootstrap>`.
 In such case use the path to your bootstrap 
 ``/var/www/nailgun/bootstraps/<your-bootstrap>``
 instead of ``/var/www/nailgun/bootstraps/active_bootstrap``

...
.. warning:: Operating on active bootstrap might follow to the case when
 a suddenly rebooted node will fail to download bootstrap images ``initrd``
 and ``root.squashfs``. This rebooting node will repeat the attempts until
 the bootstrap files have been updated (and downloaded successfully).
 You may as well create a new bootstrap image (with the command
 ``fuel-bootstrap build`` and ``fuel-bootstrap import``), modify the
 ``initrd`` and  ``root.squashfs`` files as described below and activate
 this new image (with the command ``fuel-bootstrap active <your-bootstrap>``).
 In such case use  the path to your bootstrap ``/var/www/nailgun/bootstraps/<your-bootstrap>``
 instead of ``/var/www/nailgun/bootstraps/active_bootstrap`` in the commands below.
